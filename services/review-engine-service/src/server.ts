...
export async function buildServer() {
  const server = Fastify({
    ...
    bodyLimit: 2_097_152, // 2MB — reviews may include embedded photo metadata
  });
  ...
  await server.register(createAuthPlugin, {
    publicRoutes: ['/health', '/ready', '/api/v1/reviews/status'],
    publicPrefixes: ['/api/v1/reviews/property/'],
  });
