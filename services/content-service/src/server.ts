...
export async function buildServer() {
  ...
  // ── Authentication ─────────────────────────────────────────────
  // Content is mostly public-readable; admin-only writes are enforced
  // at the route level via preHandler hooks.
  await server.register(createAuthPlugin, {
    publicRoutes: ['/health', '/ready', '/api/v1/content/status'],
    publicPrefixes: [
